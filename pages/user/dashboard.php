...
<body>
            ...
            <div class="main-dashboard-content">
                        ...
                        <div class="row g-0">
                            ...
                            <div class="col-12 col-xl-4">
                                <div class="card-body d-flex flex-column justify-content-center h-100 p-4">
                                    ...
                                    <div
                                        class="d-flex flex-column flex-xl-row justify-content-between align-items-xl-end">
                                        <div class="mb-3 mb-xl-0 flex-grow-1">
                                            <p class="text-muted mb-1">Get started by adding your courses for the
                                                semester.</p>
                ...
                <div class="px-3">
                    ...
                    <div class="class-item-custom d-flex align-items-center justify-content-between mb-2">
                        ...
                        <button class="btn btn-custom-outline ms-3 flex-shrink-0 btn-outline-secondary rounded-pill">
                            <span>View</span>
                        </button>
                    </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <!-- Step 1: Upload -->
                    <div id="upload-step">
                        <h6 class="fw-bold text-primary mb-3">Step 1: Upload your PDF file</h6>
                        <div class="input-group mb-3 shadow-sm">
                            <input type="file" class="form-control" id="studyLoadPdf" accept="application/pdf">
                            <label class="input-group-text bg-primary text-white fw-medium"
                                for="studyLoadPdf">Upload</label>
                        </div>
                        <button class="btn btn-primary shadow-sm" id="processOcrBtn">Process Document</button>
                    </div>
                    ...
                    <div id="preview-step" style="display: none;">
                        <h6 class="fw-bold text-primary mb-3">Step 2: Preview Extracted Schedule</h6>
                        <div id="preview-content" class="p-3 border rounded mb-3 bg-light"
                            style="max-height: 400px; overflow-y: auto;">
                            <p class="text-center text-muted">Awaiting file upload...</p>
                        </div>
                        <div class="d-flex">
                            <button class="btn btn-outline-secondary me-2 shadow-sm" id="backToUploadBtn">Back</button>
                            <button class="btn btn-success shadow-sm" id="confirmScheduleBtn">Confirm Extracted
                                Schedule</button>
                        </div>
                    </div>
                    ...
                    <!-- Step 3: Confirmation -->
                    <div id="confirmation-step" style="display: none;" class="text-center">
                        <h6 class="fw-bold text-primary mb-3">Step 3: Confirmation</h6>
                        <div class="alert alert-success mb-3">
                            <i class="fas fa-check-circle me-2"></i>
                            Your study load has been successfully saved!
                        </div>
                        <button class="btn btn-success shadow-sm" data-bs-dismiss="modal">Done</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        // Note: This modal has 3 steps. The flow is: Upload -> Preview -> Confirmation

        // Process OCR button click
        $("#processOcrBtn").click(function () {
            let file = $("#studyLoadPdf")[0].files[0];
            if (!file) {
                showOcrAlert("Please upload a file first.", "danger");
                return;
            }
            ...
            showOcrAlert("Processing document...", "info");
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        ...
                        $("#upload-step").hide();
                        $("#preview-step").show();
                        showOcrAlert("Document processed successfully! Please review the extracted schedule.", "success");
                        ...
                        response.schedule.forEach(item => {
                            scheduleHtml += `<tr>
                                                <td>${item.sched_no || 'N/A'}</td>
                                                <td>${item.course_no || 'N/A'}</td>
                                                <td>${item.time || 'N/A'}</td>
                                                <td>${item.days || 'N/A'}</td>
                                                <td>${item.room || 'N/A'}</td>
                        ...
                        showOcrAlert(response.error || "An error occurred during processing.", "danger");
                    }
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    showOcrAlert("An unexpected error occurred. Please try again.", "danger");
        ...
        $("#confirmScheduleBtn").click(function () {
            // Here you would typically send the confirmed schedule to the server
            // For now, we'll just show the confirmation step

            // Hide preview step, show confirmation step
            $("#preview-step").hide();
            $("#confirmation-step").show();

            // In a real application, you would send an AJAX request here:
            // $.post("../../pages/user/save_schedule.php", { schedule: scheduleData }, function(response) {
            //     // Handle response
            // });
        });

        // Helper function to show OCR alerts
        function showOcrAlert(message, type) {
            $("#ocr-alert").removeClass("d-none alert-success alert-danger alert-info")
                .addClass("alert-" + type)
                .text(message);
        }

        // Helper function to hide OCR alert
        ...
        $("#restartOnboardingBtn").click(function () {
            $.post("../../pages/user/restart_onboarding.php", {
                user: "<?= $user['id'] ?? 0 ?>"
            }, function (data) {
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            window.addEventListener('resize', function () {
                if (screenSizeText) updateScreenSizeIndicator();
