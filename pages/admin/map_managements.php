...
require_once('../../templates/footer.php'); // Added this, assuming it was missing or moved
...
// Define floor names for display
$floor_names = [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        ...
        if ($action === 'update_room_status') {
            ...
            $is_available = (int) $_POST['is_available'];
            $floor = $_POST['floor']; // NEW: Get floor

            $stmt = $pdo->prepare("UPDATE rooms SET room_name = ?, capacity = ?, room_type = ?, location_description = ?, is_available = ?, floor = ? WHERE id = ?");
            $stmt->execute([$room_name, $capacity, $room_type, $location_description, $is_available, $floor, $room_id]); // NEW: Added floor
            ...
            $stmt = $pdo->prepare("INSERT INTO rooms (room_name, capacity, room_type, location_description, is_available, floor) VALUES (?, ?, ?, ?, 1, ?)");
            $stmt->execute([$room_name, $capacity, $room_type, $location_description, $floor]); // NEW: Added floor
...
// Fetch all rooms (updated to include 'floor')
try {
    $pdo = get_db_connection();
    $stmt = $pdo->query("SELECT id, room_name, capacity, room_type, location_description, is_available, floor FROM rooms ORDER BY room_name");
...
<body>
            ...
            <div class="map-viewer-section">
                <div class="map-viewer-header">
                    ...
                    <div class="floor-selector" id="floorSelector">
                        <?php foreach ($floors as $key => $file): ?>
                            <button 
                ...
                <div class="map-display-container">
                    <div id="mapContainer">
                        <iframe id="campusMap" src="<?= $initial_map_src ?>"
                ...
                <table class="rooms-table">
                    ...
                    <tbody id="roomsTableBody">
                        <?php foreach ($rooms as $room): ?>
                            <tr data-room-id="<?= $room['id'] ?>" data-room-data='<?= json_encode($room) ?>'>
                                <td><?= htmlspecialchars($room['room_name']) ?></td>
                                <td><?= $room['room_type'] ?></td>
                                <td><?= $floor_names[$room['floor'] ?? ''] ?? 'N/A' ?></td> <td><?= $room['capacity'] ?? 'N/A' ?></td>
        ...
        <div class="modal-content">
            ...
            <form id="roomForm" onsubmit="saveRoom(event)">
                ...
                <div class="form-group">
                    ...
                    <select id="roomFloor" name="floor" required>
                        <?php foreach ($floors as $key => $name): ?>
                             <option value="<?= $key ?>"><?= $floor_names[$key] ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <textarea id="location" name="location_description"
                        placeholder="e.g., Near the elevator, 3rd Floor, Main Building, Wing A"></textarea>
                </div>
    ...
    <script>
        ...
        let currentMapSrc = '<?= $initial_map_src ?>';
        ...
        function openEditRoomModal(roomId) {
            ...
            // Find the room data from the table (uses JSON encoded data attribute)
            const row = document.querySelector(`tr[data-room-id="${roomId}"]`);
            if (row) {
                const roomData = JSON.parse(row.dataset.roomData);
                
                // Populate the form fields
                document.getElementById('roomName').value = roomData.room_name;
                document.getElementById('roomType').value = roomData.room_type;
                document.getElementById('roomFloor').value = roomData.floor; // NEW: Set floor
                document.getElementById('capacity').value = roomData.capacity;
                document.getElementById('location').value = roomData.location_description;
                document.getElementById('isAvailable').checked = roomData.is_available == 1;
            }
        ...
        function saveRoom(event) {
            ...
            // Ensure is_available is set even if unchecked (PHP side handles this for POST)
            if (!document.getElementById('isAvailable').checked) {
                formData.append('is_available', 0);
            }
            // If capacity is empty, set it to 0 for int conversion
            ...
            fetch(window.location.href, {
                method: 'POST',
                body: new URLSearchParams(formData) // Use URLSearchParams for simple POST data
            })
        ...
        // NEW: Floor Selection Logic
        document.getElementById('floorSelector').addEventListener('click', function (e) {
            ...
            if (floorButton) {
                ...
                // Update map source and state
                currentMapSrc = floorButton.dataset.mapSrc;
                currentFloor = floorButton.dataset.floorKey;
                document.getElementById('campusMap').src = currentMapSrc;
        ...
        // Map Zoom Functionality (Kept original logic, just moved it inside DOMContentLoaded if possible)
        let currentZoom = 1;
        const minZoom = 0.5;
        const maxZoom = 3;
        const zoomStep = 0.2;
        ...
        function applyZoom() {
            ...
            // Adjust height based on zoom level (to maintain scrollability)
